...
void Sketchpad::Draw() const {
  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      size_t index = row * num_pixels_per_side_ + col;
      if (sketchpad_image_.at(index) == '+' ||
          sketchpad_image_.at(index) == '#') {
  ...
  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      ...
      if (glm::distance(brush_sketchpad_coords, pixel_center) <=
          brush_radius_) {
        size_t index = row * num_pixels_per_side_ + col;
        sketchpad_image_.replace(index, 1, "#");
      }
    }
  }
}
...
void Sketchpad::Clear() {
  sketchpad_image_ = "";
  for (size_t index = 0; index < num_pixels_per_side_ * num_pixels_per_side_;
       ++index) {
    sketchpad_image_ += " ";
  }
}
const std::string Sketchpad::GetSketchpadImage() {
